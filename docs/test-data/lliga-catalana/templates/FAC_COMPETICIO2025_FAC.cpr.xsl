    ...
    <table class="fs_res">
      <thead>
        <tr class="fs_res_res_row" onmouseover="this.className = 'hover'" onmouseout="this.className='fs_res_res_row'" >
          <th class="fs_res">Màniga</th>
          <th class="fs_res">Data</th>
          <th class="fs_res">
            Distància [km]
          </th>
          <th class="fs_res">
            ...
            <td class="fs_res">
              <xsl:choose>
                <xsl:when test="$es and $no_of_startgates = 1">
                  <xsl:text>Cursa a Gol amb </xsl:text>
                </xsl:when>
                <xsl:when test="$es and $no_of_startgates > 1">
                  <xsl:text>Race to goal with </xsl:text>
                  <xsl:value-of select="$no_of_startgates"/>
                  <xsl:text> Start Points</xsl:text>
                </xsl:when>
                <xsl:when test="$es and $no_of_startgates = 0">
                  <xsl:text>Temps individual</xsl:text>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:text>Distància Lliure</xsl:text>
                </xsl:otherwise>
              </xsl:choose>
            </td>
          </tr>
  ...
  <xsl:template match="/">
    <html>
      ...
      <body>
        <div>
          <div class="fs_res">
            <img src="https://federacioaeria.cat/wp-content/uploads/2021/04/civl-logo.png"/>
            <h1>
            ...
            <xsl:choose>
              <xsl:when test="$top_x_tasks='all'">
                <h3>
                  Resultats Totals <xsl:if test="$discard_factor > 0">
              ...
              <xsl:otherwise>
                <h3>
                  Resultats fent servir les <xsl:value-of select="$top_x_tasks"/> millors mànigues de cada pilot
                </h3>
              </xsl:otherwise>
            </xsl:choose>
            <p>
                ...
                <b>
                  <xsl:choose>
                    <xsl:when test="$top_x_tasks = 1">
                      Només es fa servir la millor màniga de cada pilot per la classificació total.
                    </xsl:when>
                    <xsl:otherwise>
                      Només les <xsl:value-of select="$top_x_tasks"/> millors mànigues puntuades de cada pilot es fan servir per a la puntuació total.
                    </xsl:otherwise>
                  </xsl:choose>
                </b>
              </p>
            ...
            <table class="fs_res">
              <thead>
                <tr class="fs_res_res_row" onmouseover="this.className = 'hover'" onmouseout="this.className='fs_res_res_row'" >
                  <th class="fs_res">#</th>
                  <th class="fs_res">Dorsal</th>
                  <th class="fs_res">Nom</th>
                  <th class="fs_res">M/F</th>
                  <th class="fs_res">Nac.</th>
                  <th class="fs_res">Vela</th>
                  <th class="fs_res">Categ.</th>
                  <xsl:for-each select="$comp_result/FsParticipant[1]/FsTask">
                    <th class="fs_res">
                      <xsl:text>T </xsl:text>
            ...
            <xsl:if test="not($result_id = 'overall')">
              <p>
                Els resultats inclouen només aquells pilots on <xsl:value-of select="$result_id"/>
              </p>
            </xsl:if>
          </div>
          <div class="fs_res" style="display: inline; width:100%;font-size: xx-small;" >
            <i>
              Classificació creada: <xsl:value-of select="$comp_result/@ts"/>
            </i>
          </div>
          <div class="fs_res" style="width:100%;font-size: x-small;" >
            <p>
              FS development, maintenance and support by <a href="https://www.volirium.com" target="_new">
                <img height="20" src="https://www.volirium.com/wp-content/uploads/2017/09/favicon.png"/>
              </a>
            </p>
          </div>
        </div>
