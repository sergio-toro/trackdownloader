import MANUFACTURERS from "flight-recorder-manufacturers";

function lookupManufacturer(id: string) {
  const short = id.length === 1;

  id = id.toUpperCase();

  const manufacturers = MANUFACTURERS.filter(
    (it) => it[short ? "short" : "long"] === id
  );
...
const RE_TASK =
  /^C(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})(\d{4})([-\d]{2})(.*)/;
const RE_TASKPOINT =
...
class IGCParser {
  ...
  private parseDateHeader(line: string): {
    date: string;
    numFlight: number | null;
  } {
  ...
  private parseTextHeader(
    ...
    if (!match) {
      throw new Error(
        `Invalid ${headerType} header at line ${this.lineNumber}: ${line}`
      );
    }

    const dataSource = match[1];
    if (
      VALID_DATA_SOURCES.indexOf(dataSource) === -1 &&
      !this.options.lenient
    ) {
      throw new Error(
        `Invalid data source at line ${this.lineNumber}: ${dataSource}`
      );
    }

    return (match[2] || match[3] || "")
      .replace(/_/g, underscoreReplacement)
      .trim();
  }
  ...
  private parseTask(line: string): IGCParser.Task {
    ...
    if (!match) {
      throw new Error(
        `Invalid task declaration at line ${this.lineNumber}: ${line}`
      );
    }
    ...
    const declarationTimestamp = Date.parse(
      `${declarationDate}T${declarationTime}Z`
    );
  ...
  private parseTaskPoint(line: string): IGCParser.TaskPoint {
    ...
    if (!match) {
      throw new Error(
        `Invalid task point declaration at line ${this.lineNumber}: ${line}`
      );
    }

    const latitude = IGCParser.parseLatitude(
      match[1],
      match[2],
      match[3],
      match[4]
    );
    const longitude = IGCParser.parseLongitude(
      match[5],
      match[6],
      match[7],
      match[8]
  ...
  private parseBRecord(line: string): IGCParser.BRecord {
    ...
    const pressureAltitude =
      match[13] === "00000" ? null : parseInt(match[13], 10);
    ...
    let enl = null;
    if (extensions["ENL"]) {
      const enlLength = this.fixExtensions.filter((it) => it.code === "ENL")[0]
        .length;
    ...
    const fixAccuracy = extensions["FXA"]
      ? parseInt(extensions["FXA"], 10)
      : null;
  ...
  private parseIJRecord(line: string): IGCParser.RecordExtension[] {
    ...
    if (!match) {
      throw new Error(
        `Invalid ${line[0]} record at line ${this.lineNumber}: ${line}`
      );
    }
